...
import {
  ...
  RotateCcw, ChevronRight, X, Barcode,
} from 'lucide-react'
...
interface ApiProduct {
  ...
  supplier: string | null; unit_name: string | null
}
...
function toCartProduct(p: ApiProduct): Product {
  return {
    ...
    price: parseFloat(p.price), cost: 0, tax_rate: 0,
...
export default function PosPage() {
  ...
  const { data: productsData } = useQuery({
    ...
    queryFn: () => apiGet<ProductsApiResponse>('/products', {
  ...
  const checkoutMutation = useMutation({
    ...
    onSuccess: (res: unknown) => {
      ...
      toast.success(`Sale #${num} completed!`)
      cart.clearCart()
      setPayModal(false)
      setTenderedAmount('')
    ...
    onError: (err: unknown) => {
      const msg = (err as { response?: { data?: { message?: string } } })?.response?.data?.message ?? 'Sale failed. Please retry.'
      toast.error(msg)
    },
  })
      ...
      <div className="flex flex-1 flex-col bg-gray-50 dark:bg-gray-900 overflow-hidden">
        ...
        <div className="p-4 space-y-3 bg-white dark:bg-gray-800 border-b dark:border-gray-700">
          <div className="relative">
            ...
            />
            {search && (
              <button className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400" onClick={() => setSearch('')}>
                <X className="h-4 w-4" />
              </button>
            )}
          </div>
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            <button
              ...
              className={clsx('flex-shrink-0 rounded-full px-3 py-1 text-xs font-medium transition-colors',
                !category ? 'bg-primary-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300')}
            >All</button>
            {categories.map((cat) => (
              <button
                key={cat}
                ...
                className={clsx('flex-shrink-0 rounded-full px-3 py-1 text-xs font-medium transition-colors',
                  category === cat ? 'bg-primary-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300')}
              >{cat}</button>
            ))}
          </div>
        </div>
          ...
          {products.map((apiProd) => {
            ...
            return (
              ...
              >
                ...
                <p className="mt-0.5 text-xs text-gray-400">{apiProd.barcode ?? apiProd.category ?? ''}</p>
                ...
                {inCart && (
                  <div className="mt-1 text-center text-xs font-semibold text-primary-600 bg-primary-50 dark:bg-primary-900/20 rounded py-0.5">
      ...
      <div className="w-full max-w-xs lg:max-w-sm flex flex-col bg-white dark:bg-gray-800 border-l dark:border-gray-700">
        ...
        <div className="p-4 border-b dark:border-gray-700 flex items-center justify-between">
          <span className="font-semibold text-gray-900 dark:text-white flex items-center gap-2">
            <ShoppingCart className="h-5 w-5" /> Cart ({cart.items.length})
          </span>
          <div className="flex gap-1">
            <button title="Hold order" onClick={cart.holdOrder} disabled={cart.items.length === 0}
              className="p-1.5 rounded-md text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 disabled:opacity-40">
              <Pause className="h-4 w-4" />
            </button>
            <button title="Clear cart" onClick={cart.clearCart} disabled={cart.items.length === 0}
            ...
            <div key={item.product_id} className="px-4 py-3 flex items-center gap-3">
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => cart.updateQuantity(item.product_id, item.quantity - 1)}
                  className="h-6 w-6 rounded-md bg-gray-100 dark:bg-gray-700 flex items-center justify-center text-gray-600 hover:bg-gray-200">
              ...
              <button onClick={() => cart.removeItem(item.product_id)} className="text-red-400 hover:text-red-600">
          ...
          <div className="w-full max-w-md rounded-2xl bg-white dark:bg-gray-800 shadow-2xl">
            ...
            <div className="p-5 space-y-4">
              <div className="grid grid-cols-3 gap-2">
                {(['cash', 'card', 'wallet'] as const).map((m) => (
                  <button key={m} onClick={() => cart.setPaymentMethod(m)}
                    className={clsx('rounded-lg py-2.5 text-sm font-medium capitalize transition-colors',
                      cart.payment_method === m ? 'bg-primary-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200 dark:bg-gray-700 dark:text-gray-300')}>
                    {m}
                  </button>
                ))}
              </div>
              <div className="text-center text-3xl font-bold text-gray-900 dark:text-white">{total.toFixed(2)}</div>
                ...
                <>
                  <div>
                    ...
                    <input type="number" value={tenderedAmount} onChange={(e) => setTenderedAmount(e.target.value)}
                      placeholder="0.00" className="input text-lg text-center font-semibold" autoFocus />
                  </div>
                  {tendered > 0 && (
                    <div className={clsx('rounded-lg p-3 text-center font-semibold',
              ...
              <button
                ...
                disabled={checkoutMutation.isPending || (cart.payment_method === 'cash' && tendered < total)}
                className="w-full btn-primary py-3 text-base font-semibold disabled:opacity-50"
              >
                {checkoutMutation.isPending ? 'Processing…' : `Confirm Sale — ${total.toFixed(2)}`}
              </button>
            </div>
          </div>
        </div>
