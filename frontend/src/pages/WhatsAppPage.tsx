...
function statusBadge(s: string): string {
  ...
  if (s === 'read') return 'badge badge-info' // purple via inline style override
  ...
  if (!canManage) {
    return (
      <div className="card p-8 text-center text-gray-400 space-y-3">
        ...
        <p className="font-medium">WhatsApp module not accessible</p>
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
          <MessageSquare className="h-6 w-6 text-green-500" /> WhatsApp
        </h1>
      </div>
      ...
      <div className="flex gap-1 p-1 bg-gray-100 dark:bg-gray-700 rounded-lg w-fit flex-wrap">
        {([
          ...
          { key: 'debt_reminders', label: 'Debt Reminders', icon: Bell },
        ...
        <div className="space-y-4">
          {statsLoading ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              ...
              <div className="card p-5 space-y-1">
                <p className="text-xs text-gray-500 uppercase font-semibold">Sent Today</p>
                <p className="text-3xl font-bold text-green-600">{stats?.sent_today?.toLocaleString() ?? '—'}</p>
              </div>
              <div className="card p-5 space-y-1">
                <p className="text-xs text-gray-500 uppercase font-semibold">Inbound Today</p>
                <p className="text-3xl font-bold text-blue-600">{stats?.inbound_today?.toLocaleString() ?? '—'}</p>
              </div>
              <div className="card p-5 space-y-1">
                <p className="text-xs text-gray-500 uppercase font-semibold">Failed Today</p>
      ...
      {tab === 'logs' && (
        <div className="space-y-4">
          ...
          <div className="card p-4 flex flex-wrap gap-3 items-end">
            ...
            <button onClick={clearLogFilters} className="btn btn-secondary self-end">Clear</button>
          </div>
              ...
              <>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 dark:bg-gray-700">
                      <tr>{['To', 'Type', 'Status', 'Direction', 'Date', 'Error'].map((h) => <th key={h} className="px-4 py-3 text-left text-xs font-semibold uppercase text-gray-500">{h}</th>)}</tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                      {logs.length === 0
                        ? <tr><td colSpan={6} className="px-4 py-10 text-center text-gray-400">No messages found</td></tr>
                        : logs.map((log) => (
                ...
                {totalPages > 1 && (
                  <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100 dark:border-gray-700">
                    <p className="text-sm text-gray-500">Page {logPage} of {totalPages} · {logsTotal} total</p>
                    <div className="flex gap-2">
                      <button disabled={logPage <= 1} onClick={() => setLogPage((p) => p - 1)} className="btn btn-secondary text-xs py-1 px-3 disabled:opacity-40">Prev</button>
                      <button disabled={logPage >= totalPages} onClick={() => setLogPage((p) => p + 1)} className="btn btn-secondary text-xs py-1 px-3 disabled:opacity-40">Next</button>
                    </div>
                  </div>
                )}
              </>
      ...
      {tab === 'promotions' && (
        <div className="max-w-2xl space-y-4">
          <div className="card p-6 space-y-4">
            <h2 className="font-semibold text-gray-900 dark:text-white flex items-center gap-2"><Megaphone className="h-4 w-4 text-green-500" /> Send Promotion</h2>
            <form onSubmit={handlePromoSend} className="space-y-4">
              ...
              <label className="flex items-center gap-2 cursor-pointer select-none">
                ...
                <span className="text-sm text-gray-700 dark:text-gray-300">VIP Customers Only</span>
              </label>
              <button type="submit" disabled={promoMutation.isPending || promoMessage.length > 1000} className="btn btn-primary flex items-center gap-2">
                ...
                {promoMutation.isPending ? 'Sending…' : 'Send Promotion'}
              </button>
            </form>
          </div>
      ...
      {tab === 'debt_reminders' && (
        <div className="max-w-xl space-y-4">
          <div className="card p-6 space-y-4">
            <h2 className="font-semibold text-gray-900 dark:text-white flex items-center gap-2"><Bell className="h-4 w-4 text-orange-500" /> Bulk Debt Reminders</h2>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              This will send payment reminders to all customers with outstanding balances. Use this feature carefully — each customer will receive a WhatsApp message with their current debt amount.
            </p>
            <button
            ...
            >
              ...
              {debtMutation.isPending ? 'Sending…' : 'Send Bulk Reminders'}
            </button>
          </div>
      ...
      <ConfirmDialog
        open={bulkConfirm}
        title="Send Bulk Debt Reminders"
